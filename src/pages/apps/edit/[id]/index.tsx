...
import { useRoqClient, useAppFindFirst } from 'lib/roq';
...
function AppEditPage() {
  ...
  const { data, error, isLoading, mutate } = useAppFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: AppInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.app.update({
        data: values as any,
        where: {
          id,
        },
