...
import { useRoqClient, useAppFindManyWithCount } from 'lib/roq';
import { convertQueryToPrismaUtil } from 'lib/utils';

type ColumnType = ColumnDef<AppInterface, unknown>;
...
export function AppListPage(props: AppListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['company', 'user', 'bug.count', 'feature.count', 'test_case.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['name.contains', 'version.contains', 'platform.contains'],
          ...params.filters,
        },
        'app',
      ),
  ...
  const { data, error, isLoading, mutate } = useAppFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.app.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('feature', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Feature',
          accessorKey: 'feature',
          cell: ({ row: { original: record } }: any) => record?._count?.feature,
        }
      : null,
    hasAccess('test_case', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.test_case,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/apps/view/${record.id}`} passHref legacyBehavior>
                <Button
