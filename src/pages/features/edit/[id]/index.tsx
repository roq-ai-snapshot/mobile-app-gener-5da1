...
import { useRoqClient, useFeatureFindFirst } from 'lib/roq';
...
function FeatureEditPage() {
  ...
  const { data, error, isLoading, mutate } = useFeatureFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: FeatureInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.feature.update({
        data: values as any,
        where: {
          id,
        },
