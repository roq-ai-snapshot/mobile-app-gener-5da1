...
import { useAppCount, useTestCaseCount, useFeatureCount } from 'lib/roq';

import { AppListPage as ShowCaseList } from 'pages/apps';

function DashboardPage() {
  const { session } = useSession();

  const appStat = useAppCount();

  const testCaseStat = useTestCaseCount();

  const featureStat = useFeatureCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Apps',
      result: appStat?.data,
      isLoading: appStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Test Cases',
      result: testCaseStat?.data,
      isLoading: testCaseStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Features',
      result: featureStat?.data,
      isLoading: featureStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
